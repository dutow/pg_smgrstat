RSpec.describe "pg_smgrstat temporary table tracking" do
  ...
  describe "mode: off" do
    ...
    it "does not track temp table I/O" do
      ...
      result = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE relnumber = #{relfilenode}")
      expect(result[0]["n"].to_i).to eq(0)

      # Also check that no aggregate entry exists
      result = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")
      expect(result[0]["n"].to_i).to eq(0)
    end
  end
  ...
  describe "mode: individual" do
    ...
    it "tracks each temp table separately" do
      ...
      result1 = stats_conn.exec("SELECT writes, write_blocks FROM smgr_stats.current() WHERE relnumber = #{relfilenode1} AND forknum = 0")
      result2 = stats_conn.exec("SELECT writes, write_blocks FROM smgr_stats.current() WHERE relnumber = #{relfilenode2} AND forknum = 0")
    ...
    it "resolves temp table metadata correctly" do
      ...
      result = stats_conn.exec("SELECT relname, nspname, relkind, writes, write_blocks FROM smgr_stats.current() WHERE relnumber = #{relfilenode} AND forknum = 0")
  ...
  describe "mode: aggregate (default)" do
    ...
    it "combines all temp tables into one entry" do
      ...
      result1 = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE relnumber = #{relfilenode1}")
      result2 = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE relnumber = #{relfilenode2}")
      ...
      result = stats_conn.exec("SELECT writes, write_blocks FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")
      expect(result.ntuples).to eq(1)
      expect(result[0]["write_blocks"].to_i).to be > 0
    end
    ...
    it "shows special metadata for aggregate entry" do
      ...
      result = stats_conn.exec("SELECT relname, nspname, relkind FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")
      expect(result.ntuples).to eq(1)
    ...
    it "accumulates stats across multiple temp tables" do
      ...
      first_writes = stats_conn.exec("SELECT write_blocks FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")[0]["write_blocks"].to_i
      ...
      second_writes = stats_conn.exec("SELECT write_blocks FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")[0]["write_blocks"].to_i
  ...
  describe "mode switching mid-session" do
    it "affects only subsequent I/O" do
      ...
      agg_result = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE spcoid = 0 AND relnumber = 0")
      ...
      ind_result = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE relnumber = #{relfilenode2} AND forknum = 0")
      ...
      off_result = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.current() WHERE relnumber = #{relfilenode3}")
...
RSpec.describe "pg_smgrstat temp table history collection",
  ...
  it "collects only aggregate entry in history, not individual temp tables" do
    ...
    agg_result = stats_conn.exec(<<~SQL)
      SELECT count(*) AS n
    ...
    ind_result1 = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.history WHERE relnumber = #{relfilenode1}")
    ind_result2 = stats_conn.exec("SELECT count(*) AS n FROM smgr_stats.history WHERE relnumber = #{relfilenode2}")
